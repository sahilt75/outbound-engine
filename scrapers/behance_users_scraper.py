...
# -------------------------
async def extract_user_data(page: Page, profile_url: str) -> Dict:
    out = {
        "profile_name": "",
        "username": "",
    ...
    # optionally store raw html for debugging
    try:
        out["raw_html"] = await page.content()
    except Exception:
        out["raw_html"] = ""
...
# Main flow
# -------------------------
async def run_user_scrape(field: str, country: str, max_users: int, output: Path, headless: bool = True):
    ...
    async with async_playwright() as p:
        ...
        for idx, profile in enumerate(users_collected, start=1):
            try:
                ...
                data = await extract_user_data(page, profile)
...
def parse_args():
    ...
    ap.add_argument("--output", type=str, default="out/behance_users_output.csv", help="Output CSV path (also writes JSON)")
...
def main():
    ...
    asyncio.run(run_user_scrape(args.field, args.country, args.max_users, out_path, headless=headless_flag))
